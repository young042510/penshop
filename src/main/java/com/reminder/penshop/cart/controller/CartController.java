package com.reminder.penshop.cart.controller;

import com.reminder.penshop.cart.model.dao.CartMapper;
import com.reminder.penshop.cart.model.dto.CartDTO;
import com.reminder.penshop.cart.model.service.CartService;
import com.reminder.penshop.product.model.dao.ProductMapper;
...
public class CartController {
    ...
    public void getMyCart(HttpSession session, Model model) {
        session.removeAttribute("loginMember"); // 주문 요청시마다 주문목록 session 갱싱
    ...
    public String addToCart(HttpServletRequest request, HttpSession session, Model model) {
        ...
        if(loginMember == null && session.getAttribute("penshopCart") == null) {
            log.info("비회원용 session 장바구니 생성");
            List<CartDTO> nonmemberCart = new ArrayList<>();
            session.setAttribute("penshopCart", nonmemberCart);
        ...
        log.info("최종 cart items : {}", session.getAttribute("penshopCart"));
