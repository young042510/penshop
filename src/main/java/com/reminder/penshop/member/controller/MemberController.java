package com.reminder.penshop.member.controller;

import com.reminder.penshop.mail.model.service.MailService;
...
import com.reminder.penshop.oauth.model.service.KakaoOAuth2Service;
import com.reminder.penshop.validator.SignUpValidator;
...
public class MemberController {
    ...
    @Autowired
    public MemberController(MemberService memberService, MailService mailService, MessageSource messageSource, PasswordEncoder passwordEncoder, SignUpValidator signUpValidator, KakaoOAuth2Service kakaoOAuth2Service) {
    ...
    @ResponseBody
    //ajax 통신에서 JSON 응답을 치르기 위해서는 어노테이션 추가, spring-web의 경우 jackson-databind 등 기본적으로 추가돼 있는 의존성들을 고려하여야 함
    ...
    public String signupForm(@AuthenticationPrincipal UserImpl user, Model model) {
        if (user != null) { // 이미 로그인된 회원 재요청하는 경우 denied 페이지로 연결
            model.addAttribute("errorMessage", "이미 로그인한 상태이거나 접근이 불가능합니다.");
     ...
     */
    @GetMapping("verify")
    public String signupEmailVerification(@ModelAttribute("token") String token, RedirectAttributes rttr
            , Model model, Locale locale) throws NotFoundException {
        String email = mailService.verificationEmail(token);
        log.info("인증 emil : {}", email);

        if (email == null || token == null) { // 이미 인증 끝난 계정 or 만료된 토큰으로 재요청
            log.info("올바르지 않은 접근");
    ...
    public String signInMember(@RequestParam(required = false) String errorTitle, @RequestParam(required = false) String errorText,
         ...
         * [Handler 이동 전 요청별 확인]
         * 누구? 바로그인  상태 회원
         * 무엇을? 1) 상세페이지 바로주문 요청 -  @SessionAttributes 어노테이션 활용해 선택값을 session상에 임시 저장
         *       2) 이전 경로가 존재 -> 이름 sessison상에 임시 저장
         * */
        ...
        String uri = request.getHeader("Referer"); // 로그인 직전 페이지 불러오기
